...
### 4. Neo4j memory (optional)

If **user-neo4j-memory** MCP is available, log this outcome for persistent context: invoke protocol **log_memory** (see {project-root}/_bmad/core/tasks/workflow.xml). Use memoryType **Outcome**, agent **John**, workflow **create-prd**, and a brief summary in observations; include metadata path to the PRD output file. This enables later workflows (e.g. Winston, Bob) to **memory_search** for prior John/PRD outcomes.
